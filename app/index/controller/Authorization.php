...
class Authorization extends Base
{
    ...
    public function add()
    {
        if (request()->isPost()) {
            ...
            if ($data['level'] == 0) {// 免费
                ...
                $pay = new Pay();
                // 异步通知地址
                $data['notify_url'] = Request::domain() . url("authorization/authPaySuccess");
                // 回调地址
                $data['return_url'] = Request::domain() . url("authorization/authPaySuccessReturn");
                // 设置缓存
     ...
     */
    public function authPaySuccessReturn()
    {
        if (request()->isGet()) {
            $pay = new Pay();
            // 接收数据
    ...
    public function updateService()
    {
        if (request()->isPut()) {
            ...
            // 授权服务只能升级不能降级
            if ($data['level'] <= $authorization['level']) {
