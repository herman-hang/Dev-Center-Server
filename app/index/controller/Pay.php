...
class Pay extends Base
{
    ...
    public function wxpay(&$data = [])
    {
        ...
        //异步通知地址
        $requests->notify_url = Request::domain() . url("pay/wxpayReturn");
        try {
            // 调用接口
...
//        $params->apiDomain = 'https://openapi.alipaydev.com/gateway.do'; // 设为沙箱环境，如正式环境请把这行注释
        ...
        $requests->notify_url = Request::domain() . url("pay/alipayNotify"); // 支付后通知地址（作为支付成功回调，这个可靠）
        $requests->return_url = Request::domain() . url("pay/alipayReturn"); // 支付后跳转返回地址
